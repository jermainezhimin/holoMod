      ...
      if (active_object_.Raycast(pinch_ray, out pinch_hit, grabObjectDistance))
        ...
        */
		...
		if(Physics.Raycast(pinch_ray, out pinch_hit)){
			Meshinator deformer = pinch_hit.collider.GetComponent<Meshinator> ();
			if (deformer) {
				...
				deformer.Impact (point, direction * force, deformer.m_ImpactShape, deformer.m_ImpactType);
				//deformer.AddForce(force, direction);
			}
